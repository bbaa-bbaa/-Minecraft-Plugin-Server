...
func (sc *ScoreboardCore) registerTrigger(context pluginabi.PluginName, goFunc func(string, int)) (name string) {
	...
	sc.Println(
		color.YellowString("插件 "),
		color.BlueString(context.DisplayName()),
		color.YellowString(" 注册了一个 (Autogenerated)"),
		color.GreenString(name),
		color.YellowString("触发器"),
	)
	sc.tlock.Lock()
	sc.trigger[name] = goFunc
	sc.tlock.Unlock()
	sc.RunCommand(strings.Join([]string{fmt.Sprintf("scoreboard objectives add %s trigger", name), fmt.Sprintf("scoreboard players enable @a %s", name)}, "\n"))
	return name
}
...
func (sc *ScoreboardCore) clearTrigger() {
	...
	if len(triggerStrList) == 2 {
		...
		for _, trigger := range triggerStrList {
			if strings.Contains(trigger, "_m_trigger_") {
